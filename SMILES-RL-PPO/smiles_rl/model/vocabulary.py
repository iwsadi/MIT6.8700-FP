...
def create_vocabulary_from_hf_tokenizer(hf_tokenizer):
    """
    Builds a Vocabulary from a Hugging Face tokenizer, preserving ids so
    transformer embeddings stay aligned with the checkpoint.
    
    Also adds aliases '^' and '$' pointing to BOS/EOS token ids for
    compatibility with the existing sampling code that expects these tokens.
    """
    vocab = Vocabulary(tokens=hf_tokenizer.get_vocab())
    ...
    # Map '^' to BOS token id for compatibility with sampling code.
    # We need to add it as an alias that points to the same id.
    if bos_id is not None and "^" not in vocab:
        # Add '^' as an alias for the BOS token
        vocab._tokens["^"] = bos_id
    
    # Map '$' to EOS token id (or pad if no EOS)
    if eos_id is None:
        eos_id = pad_id
    if eos_id is not None and "$" not in vocab:
        # Add '$' as an alias for the EOS token
        vocab._tokens["$"] = eos_id
