...
class RNN(tnn.Module):
    ...
    def forward(
        ...
        if hidden_state is None:
            ...
            # Use the device of the RNN parameters to ensure hidden state is on correct device
            # regardless of input device (since Embedding handles CPU inputs but outputs to its device)
            device = next(self.parameters()).device 
            if self._cell_type == "gru":
                hidden_state = torch.zeros(*size, device=device)
            else:
                hidden_state = [torch.zeros(*size, device=device), torch.zeros(*size, device=device)]
