...
class A2C(BaseAgent):
    def __init__(
        ...
        self.entropy_penalty = sp.get("entropy_penalty", 0.25)

        self.max_grad_norm = sp.get("max_grad_norm", 5)

        self.tau = sp.get("tau", 0.99)

        self.learning_rate_critic = sp.get(
            "learning_rate_critic", self.config.learning_rate
        )

        self.learning_rate_actor = sp.get(
            "learning_rate_actor", self.config.learning_rate
        )

        # Actor selection: "rnn" (default) or "transformer"
        self.actor_type = sp.get("actor_type", "rnn").lower()
        self.transformer_weights = sp.get("transformer_weights", None)
        self.transformer_network: Optional[dict] = sp.get(
            "transformer_network",
            {"layer_size": 256, "n_layers": 6, "n_heads": 8, "dropout": 0.0},
        )
        self.freeze_transformer_embeddings = sp.get(
            "freeze_transformer_embeddings", False
        )
        self.max_sequence_length_override = sp.get("max_sequence_length", None)
        if self.actor_type not in ("rnn", "transformer"):
            raise ValueError(f"Unsupported actor_type '{self.actor_type}'. Use 'rnn' or 'transformer'.")
    ...
    def reset(
        ...
        if self.actor_type == "transformer":
            self._actor = ActorModelTransformer.load_from_file(
                ...
                sampling_mode=False,
                network_params_override=self.transformer_network,
                freeze_embeddings=self.freeze_transformer_embeddings,
                max_sequence_length=self.max_sequence_length_override,
            )
        ...
        # Critic stays the original RNN-based value network
